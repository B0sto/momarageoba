...
const FeaturedCard = ({ name, itemCount, image }: FeaturedCardProps) => {
  ...
  return (
    <div className='rounded-[10px] p-6 border border-[#F4F6FA] bg-white flex flex-col items-center'>
      {image && <img src={image} alt={displayName} className='w-16 h-16 object-cover mb-4' />}
      <h4 className='mt-4 font-quicksand font-bold leading-[120%] text-[#254D4E]'>{displayName}</h4>
      <p className='text-[#7E7E7E] font-lato leading-[24px]'>{itemCount} items</p>
    </div>
  )
}
